...
这里仅为未完成、需要跨会话继续的工作保存恢复摘要，完成的普通任务无需创建。日常协作使用 `<topic>/<YYYYMMDD>[-<label>].md`，topic 为稳定任务名，不要求对应 OpenSpec；直接读取文件恢复。完成后标记已完成，不改写旧检查点。

以下 canonical change id 路径规则用于自动循环，保留看板与脚本兼容。
