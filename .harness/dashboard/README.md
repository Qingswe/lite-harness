...
看板是自动循环的可选工具，日常协作无需启动看板。已有状态、身份、验证与归档契约保持有效；任务和进度直接查询 OpenSpec。

集中查看并编辑各 change 的**任务复选框**与 `verification.json` 的**验证步骤**，直接在网页上勾选 / 切状态 / 填备注，写回文件。看板不降低 `harness close` 的校验门槛，也不提供直接 archive，同时只读预览归档就绪度、检查点、验证记录、证据与项目级质量/知识文档。

看板展示和写回已有任务、验证文件，不是独立的状态权威；可编辑入口不授予 QA 或 Done 判定权限。操作遵循根目录 [CLAUDE.md](../../CLAUDE.md) 及其引用的项目角色规则。就绪或归档不能代替项目约定的产品验收；必需人工验收须进入人工步骤，尚未落实的约束先记录为 blocker。
...
- **生命周期**：从任务与验证记录推导，不手工维护 phase 或候选成员关系。

**只读预览：**

- **概览**：OpenSpec change、任务进度、验证结论和阻塞，刷新时实时计算。
...
- **分层导航**：侧栏是服务端派生的三层折叠树（`/api/state` 的 `nav_tree`）。一级分组首屏展开、二级收起，任意 change / 文档 / 证据集最多两次点击可达；展开状态存 `localStorage`，不写仓库文件。
...
- **安全写回**：任务复选框按行号定位 + 乐观锁（提交携带原始整行，文件被外部改动则返回 `409` 并自动刷新），只改目标行；验证步骤按**步骤标识**寻址而非行号——JSON 里步骤位置会变，标识不会，并发冲突同样返回 `409`。
...
- `server.py` —— HTTP 服务与 API（`/api/state`、`/api/ready`、`/api/task`、`/api/verification-step`、`/api/doc`、`/api/evidence-file`、`GET/POST /api/roles`），以及静态路由与 `DATA_FLOW` 声明
- `index.html` —— DOM 骨架
